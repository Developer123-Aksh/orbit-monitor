...
class ModbusMaster {
  ...
  /**
   * readInputRegisters (FC 04)
   * Reads multiple input registers (FC4).
   *
   * @param int $unitId usually ID of Modbus device 
   ...
   * @return false|Array Success flag or array of received data.
   */
  function readInputRegisters($unitId, $reference, $quantity){
    $this->status .= "readInputRegisters: START\n";
    // connect
    $this->connect();
    // send FC 4
    $packet = $this->readInputRegistersPacketBuilder($unitId, $reference, $quantity);
    $this->status .= $this->printPacket($packet);    
    $this->send($packet);
    // receive response
    $rpacket = $this->rec();
    $this->status .= $this->printPacket($rpacket);    
    // parse packet (Reuses FC3 parser as the response structure is identical)   
    $receivedData = $this->readMultipleRegistersParser($rpacket);
    // disconnect
    $this->disconnect();
    $this->status .= "readInputRegisters: DONE\n";
    // return
    return $receivedData;
  }
  ...
  /**
   * fc4
   * Alias to readInputRegisters method.
   */
  function fc4($unitId, $reference, $quantity){
    return $this->readInputRegisters($unitId, $reference, $quantity);
  }

  /**
   * readInputRegistersPacketBuilder
   * Packet FC 4 builder - read input registers
   */
  private function readInputRegistersPacketBuilder($unitId, $reference, $quantity){
    $dataLen = 0;
    $buffer1 = "";
    $buffer2 = "";
    $buffer2 .= iecType::iecBYTE(4);              // FC 4 = 4(0x04)
    $buffer2 .= iecType::iecINT($reference);      // refnumber      
    $buffer2 .= iecType::iecINT($quantity);       // quantity
    $dataLen += 5;
